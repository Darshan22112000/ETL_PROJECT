...
def get_merged_data():
    ...
    sq2 = session.query(Person)
    person = pd.read_sql(sq2.statement, session.bind)
    sq3 = session.query(Vehicle)
    vehicle = pd.read_sql(sq3.statement, session.bind)
    ...
    return df, crash, person, vehicle


# get_merged_data()

class Visualize_run_model(luigi.Task):

    def run(self):
        result = True
        try:
            df, crash, person, vehicle = get_merged_data()
            df.head()
