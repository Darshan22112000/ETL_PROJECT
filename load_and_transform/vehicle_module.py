...
from database.models import map_vehicle_columns, Vehicle
...
# read_vehicle_saveToMogo: This function is used to Read data from JSON and load to MongoDB
@op(
    # out=Out(pd.DataFrame)
    out=Out(bool)
)
def read_vehicle_saveToMogo() -> bool:
    result = True
    cwd = os.getcwd()
    path = os.path.join(cwd, 'Data', 'vehicle.json').replace("\\", '/')
    file_path = path
    db_name = "dap"
    ...
    try:
        ...
        vehicles_db = client[db_name]
        collection = vehicles_db[collection_name]

        # Discard collection if it already exists
        vehicles_db.drop_collection(collection_name)
        ...
        with open(file_path, 'r', encoding="utf-8") as file:
            ...
            try:
                ...
                IO_ops.insert_mongo_chunk_records(vehicles_db, collection_name, data)
...
# get_vehicle_fromMogo : This function is used to read Data from MongoDB and perform initial tranformation if requried.
@op(
    ins={"start": In(bool)},
    out=Out(pd.DataFrame)
)
def get_vehicle_fromMogo(start):  # > pd.DataFrame
    client = DatabaseUtil.get_mongo_client()
    db = client.dap
    collection_name = 'vehicle'
    vehicle_collection = db[collection_name]
    v_df = pd.DataFrame()
    try:
        cursor = vehicle_collection.find({})
        v_df = pd.DataFrame(list(cursor))
...
@op(
    ins={"vehicle_df": In(pd.DataFrame)},
    out=Out(VehicleDataFrame)
)
def tranform_vehicle(vehicle_df):
    try:
        df = vehicle_df
        ...
        # Transformation 3:Converting other columns to correct data type
        num_columns = ['vehicle_year', 'vehicle_occupants']
        for col in num_columns:
            df[col] = pd.to_numeric(df[col], errors='coerce')
            df[col] = df[col].fillna(0).astype(int)

        df["vehicle_model"] = df["vehicle_model"].fillna("").astype(str)

        # Transformation 4: # Validating that year values are within a reasonable range
        df = df[df['vehicle_year'].between(1900, 2100)]

        # Transformation 5 : Remove rows with NaN values in IDs columns
        columns_to_check = ['collision_id', 'vehicle_id']
...
@op(
    ins={"vehicle_df": In(VehicleDataFrame)},
    out=Out(bool)
)
def save_vehicle_to_postgresql(vehicle_df) -> bool:
    result = True
    try:
        df = vehicle_df
        table_name = 'vehicle'
...
def get_vehicle_from_postgresql(start):
    df = VehicleDataFrame
